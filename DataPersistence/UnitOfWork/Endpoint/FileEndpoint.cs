...
namespace UnitOfWork.Endpoint
{
    public static class FileEndpoint
    {
        ...
        public static async Task <IResult> FileCSV(IUnitOfWork unitOfWork, CancellationToken cancellationToken)
        {
            var csvBuilder = new StringBuilder();
            csvBuilder.AppendLine("Id,Name,Price");

            var products = await unitOfWork.repository.GetAllProductsAsync(cancellationToken);
